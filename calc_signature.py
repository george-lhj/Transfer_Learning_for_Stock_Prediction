...
def calc_signature(x: pd.DataFrame, y: pd.DataFrame, order=3):
    ...
    # repeat DATE 
    dates = np.repeat(df["DATE"].values, len(ret_cols))
    ...
    grouped = df_signature.groupby(["STOCK", "ID", "DATE", 'INDUSTRY', 'INDUSTRY_GROUP', 'SECTOR', 'SUB_INDUSTRY'])
    signatures = []
    keys = []

    for (stock, id_, date, industry, industry_group, sector, sub_industry), group in grouped:
        ...
        keys.append((stock, id_, date, industry, industry_group, sector, sub_industry))
    ...
    df_signature[["STOCK", "ID", "DATE", 'INDUSTRY', 'INDUSTRY_GROUP', 'SECTOR', 'SUB_INDUSTRY']] = keys
    
    # Reorganize columns
    cols_order = ["STOCK", "ID", "DATE", 'INDUSTRY', 'INDUSTRY_GROUP', 'SECTOR', 'SUB_INDUSTRY'] + sig_columns
